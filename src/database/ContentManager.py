...
from src.Letus.LetusContent import (LetusContent_Module, LetusContent_Section,
                                    LetusContentV2)
...
class ContentManagerV2:
    ...
    def check(self):
        self.__logger.emit('ContentManager:check:Start', '202', 'Checking content info', self.check.__name__)
        try:
            if not self.LC.sync and not self.LCp.sync:
                self.__logger.emit('ContentManager:check:SyncError', '202', 'Sync disabled', self.check.__name__)
                raise ValueError('ContentManager:check:SyncError')
        except ValueError:
            self.pull()
            return self.check()
        else:
            self.__logger.emit('ContentManager:check:Success', '200', 'Content info checked', self.check.__name__)
            return

    def pull(self):
        self.__logger.emit('ContentManager:pull:Start', '202', 'Pulling content info', self.pull.__name__)
        filter = {'key': self.LC.LP.key}
        content = self.collection.find_one(filter, sort=[('timestamp', pymongo.DESCENDING)])
        try:
            if content is None:
                self.__logger.emit('ContentManager:pull:NoPrevious', '202', 'No previous content found', self.pull.__name__)
            ...
            self.__logger.emit('ContentManager:pull:KeyError', '202', 'KeyError', self.pull.__name__)
            raise KeyError('ContentManager:pull:KeyError')
        else:
            if self.LC.hash != self.LCp.hash:
                self.__logger.emit('ContentManager:pull:HashChanged', '202', 'Hash changed', self.pull.__name__)
                self.changed = True
            self.LCp.sync = True
            self.__logger.emit('ContentManager:pull:Success', '200', 'Content info pulled', self.pull.__name__)
            return

    def push(self):
        self.__logger.emit('ContentManager:push:Start', '202', 'Pushing content info', self.push.__name__)
        try:
            self.check()
        except ValueError:
            self.__logger.emit('ContentManager:push:ValueError', '202', 'ValueError', self.push.__name__)
    ...
    def register(self):
        self.__logger.emit('ContentManager:register:Start', '202', 'Registering content info', self.register.__name__)
        LCO = self.LC.export()
        try:
            self.__logger.emit('ContentManager:register:Creating', '202', 'Creating content info', self.register.__name__)
            self.collection.insert_one(LCO)
        except WriteError:
            self.__logger.emit('ContentManager:register:WriteError', '202', 'WriteError', self.register.__name__)
            raise ValueError('ContentManager:register:WriteError')
        else:
            self.__logger.emit('ContentManager:register:Success', '200', 'Content info registered', self.register.__name__)
            return
...
class ContentHandlerV2:
    ...
    def get_content(self):
        ...
        if not isinstance(sections, bs4.ResultSet): raise Exception('sections is not ResultSet')
        self.LC.hash = ContentHandlerV2.hash(ContentHandlerV2.text_filter(str(content), hash=True, pretty=True, no_script=True))
        # print(ContentHandlerV2.text_filter(str(content), hash=True, pretty=True, no_script=True))
        LC_Sections = [self.get_section(section) for section in sections if isinstance(section, bs4.Tag)]
    ...
    def get_section(self, section: bs4.Tag) -> LetusContent_Section:
        ...
        summary = ContentHandlerV2.tag_filter(summary_div) if isinstance(summary_div, bs4.Tag) else None
        hash = ContentHandlerV2.hash(ContentHandlerV2.text_filter(str(section), hash=True, pretty=True, no_script=True))

        modules = section.find_all('li', {'id': re.compile(r'^module-[0-9]+$')})
        if not isinstance(modules, bs4.ResultSet): raise Exception('modules is not ResultSet')

        LC_Modules = [self.get_module(module) for module in modules if isinstance(module, bs4.Tag)]
    ...
    def get_module(self, module: bs4.Tag) -> LetusContent_Module:
        ...
        for _type in [types] if isinstance(types, str) else types if isinstance(types, list) else []:
        ...
        if link_a_href: link = str(link_a_href)

        # remove invisible elements
        link_object_type = module.find('span', {'class': 'accesshide'})
        if isinstance(link_object_type, bs4.Tag): link_object_type.extract()

        # get module link alt
        link_alt_span = module.find('span', {'class': 'instancename'}) if not isinstance(link_a, bs4.Tag) else None
        ...
        description = ContentHandlerV2.tag_filter(description_div) if isinstance(description_div, bs4.Tag) else None

        hash = ContentHandlerV2.hash(ContentHandlerV2.text_filter(str(module), hash=True, pretty=True, no_script=True))
        ...
        if hash:
            ...
            for tag in soup.find_all():
                ...
                for attr in tag.attrs:
                    if re.search(r'\b([0-9a-fA-F]{14,32}|random[0-9a-fA-F]{14,32}+_group|single_button[0-9a-fA-F]{14,32})\b', str(tag[attr])):
