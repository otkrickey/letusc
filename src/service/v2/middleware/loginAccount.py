...
def loginAccount(LA: LetusAccount):
    __logger = Logger()
    __logger.emit(
        "Service:Middleware:LoginAccount:Start",
        "202",
        "Account Checking Start",
        loginAccount.__name__,
    )

    if LA.email is None:
        __logger.emit(
            "Service:Middleware:LoginAccount:EmailError",
            "401",
            "Email Error",
            loginAccount.__name__,
        )
    ...
    try:
        ...
        if "LetusSession:login:__login_letus:Timeout" in str(e):
            __logger.emit(
                "Service:Middleware:LoginAccount:Timeout",
                "504",
        ...
        if "LetusSession:login:__login_letus:PasswordError" in str(e):
            __logger.emit(
                "Service:Middleware:LoginAccount:PasswordError",
                "401",
                "Password Error",
                loginAccount.__name__,
            )
        ...
        __logger.emit(
            "Service:Middleware:LoginAccount:Error",
            "500",
            "Unknown Error",
            loginAccount.__name__,
        )
        ...
        __logger.emit(
            "Service:Middleware:LoginAccount:Success",
            "200",
            "Letus Login Success",
            loginAccount.__name__,
        )
