...
def fetchAccount(LA: LetusAccount) -> LetusAccount:
    __logger = Logger()
    __logger.emit(
        "Service:Middleware:FetchAccount:Start",
        "202",
        "Account Registration Start",
        fetchAccount.__name__,
    )
    ...
    try:
        ...
        if "AccountManager:pull:NotFound" in str(e):
            __logger.emit(
                "Service:Middleware:FetchAccount:NotFound",
                "404",
                "Account Not Found",
                fetchAccount.__name__,
            )
        ...
        if "AccountManager:pull:KeyError" in str(e):
            __logger.emit(
                "Service:Middleware:FetchAccount:KeyError",
                "500",
        ...
        __logger.emit(
            "Service:Middleware:FetchAccount:Error",
            "500",
            "Unknown Error",
            fetchAccount.__name__,
        )
        ...
        __logger.emit(
            "Service:Middleware:FetchAccount:Success",
            "200",
