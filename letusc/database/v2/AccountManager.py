...
class AccountManager:
    ...
    def check(self):
        ...
        try:
            if not self.LA.sync:
                self.__logger.emit(
                    "AccountManager:check:SyncError",
                    "401",
                    "Account not synced",
                    self.check.__name__,
                )
            ...
            if self.LA.email is None:
                self.__logger.emit(
                    "AccountManager:check:EmailError",
                    "401",
                    "Email Error",
                    self.check.__name__,
                )
            ...
            if self.LA.encrypted_password is None:
                self.__logger.emit(
                    "AccountManager:check:PasswordError",
                    "401",
                    "Password Error",
                    self.check.__name__,
                )
            ...
            if (self.LA.cookie is None) or (not self.LA.cookie):
                self.__logger.emit(
                    "AccountManager:check:CookieError",
                    "401",
                    "Cookie Error",
                    self.check.__name__,
                )
    ...
    def pull(self):
        self.__logger.emit(
            "AccountManager:pull:Start", "202", "Fetching account", self.pull.__name__
        )
        filter = {"discord_id": self.LA.discord_id}
        account = self.collection.find_one(filter)
        if account is None:
            self.__logger.emit(
                "AccountManager:pull:NotFound",
                "404",
                "Account not found",
                self.pull.__name__,
            )
        ...
        try:
            ...
            if (self.LA.cookie is None) or (not self.LA.cookie):
                self.__logger.emit(
                    "AccountManager:pull:CookieError",
                    "401",
                    "Cookie Error",
                    self.pull.__name__,
                )
    ...
    def push(self):
        self.__logger.emit(
            "AccountManager:push:Start", "202", "Pushing account", self.push.__name__
        )
        try:
            self.check()
    ...
    def register(self):
        ...
        try:
            self.__logger.emit(
                "AccountManager:register:Creating",
                "202",
                "Creating account",
                self.register.__name__,
            )
            ...
            self.__logger.emit(
                "AccountManager:register:WriteError",
                "409",
                "Account could not be registered correctly",
                self.register.__name__,
            )
            raise WriteError("AccountManager:register:WriteError")
        else:
            self.__logger.emit(
                "AccountManager:register:Success",
                "200",
                "Account registered",
                "register",
            )
    ...
    def update(self):
        ...
        try:
            self.__logger.emit(
                "AccountManager:update:Updating",
                "202",
                "Updating account",
                self.update.__name__,
            )
            ...
            self.__logger.emit(
                "AccountManager:update:WriteError",
                "409",
                "Account could not be updated correctly",
                self.update.__name__,
            )
            raise WriteError("AccountManager:update:WriteError")
        else:
            self.__logger.emit(
                "AccountManager:update:Success",
                "200",
                "Account updated",
                self.update.__name__,
            )
